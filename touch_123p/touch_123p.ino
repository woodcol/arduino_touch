...
#define touchTime   40    //点击屏的时间

#define delayTime   25    //每秒20次时的两次点击的间隔时间

bool isStart = true;       //是否开启点击,默认上电即开启,当按下按键时关闭点击，再按下时启动点击

#define delayTime770ms 760     //延时宏800毫秒=40+760
#define delayTime5s    5000    //延时宏5000毫秒即5秒
...
defineTask(Task1);
 ...
 void Task1::loop()
 {
   if(isStart){
    ...
    digitalWrite(LED_BUILTIN,HIGH);//点击周期开始，led点亮
    //j10,j1,j2,j3,j4,j5,j6,j7,j8,j9,jJok依次按下
    //一个循环是，头点击顺序为1212123，间隔800ms，每个循环间隔5s
    //这里把j1～j10，jok的11个点击头插孔分为了三个123的组，三组的动作相同，都是按1212123的方式点击,三组分别为
    //j1 --> 1,j2 --> 2,j3 --> 3 ,第一组123
    //j4 --> 1,j5 --> 2,j6 --> 3 ,第二组123
    //j7 --> 1,j8 --> 2,j9 --> 3 ,第三组123
    
    //12,12,12三次
    for(int i = 0;i < 3;i++){
      //1的一个动作周期
      //1按下30ms
      digitalWrite(J1,LOW);//1
      delay(1);
      digitalWrite(J4,LOW);//1
      delay(1);
      digitalWrite(J7,LOW);//1
      delay(touchTime);
      //1不按770ms
      digitalWrite(J1,HIGH);//1
      digitalWrite(J4,HIGH);//1
      digitalWrite(J7,HIGH);//1
      delay(delayTime770ms);


      //2的一个动作周期
      //2按下30ms
      digitalWrite(J2,LOW);//1
      delay(1);
      digitalWrite(J5,LOW);//1
      delay(1);
      digitalWrite(J8,LOW);//1
      delay(touchTime);
      //2不按770ms
      digitalWrite(J2,HIGH);//1
      digitalWrite(J5,HIGH);//1
      digitalWrite(J8,HIGH);//1
      delay(delayTime770ms);
    }
    
    //3的一个动作周期
    //3按下30ms
    digitalWrite(J3,LOW);//1
    delay(1);
    digitalWrite(J6,LOW);//1
    delay(1);
    digitalWrite(J9,LOW);//1
    delay(touchTime);
    //3不按770ms
    digitalWrite(J3,HIGH);//1
    digitalWrite(J6,HIGH);//1
    digitalWrite(J9,HIGH);//1
//    delay(delayTime770ms);

    digitalWrite(LED_BUILTIN,LOW);//点击周期结束，led熄灭
    //第一个大周期结束的延时5秒
    delay(delayTime5s);
    //延时5秒结束，再次进入12,12,12,3的下一次循环
...
////快速定义，按键检测进程，这个功能是当按键按下时会循环123式点击，不按时就是每秒22次的点击
// defineTaskLoop(Task2)
// {
//   int keyType = digitalRead(key);
//  if(!keyType){
