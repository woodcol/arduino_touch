...
unsigned long delay_Mins[11] = {300,300,300,500,500,500,20000,20000,20000,100,100};          //随机间隔点击时间最小值，单位ms
unsigned long delay_Maxs[11] = {700,700,700,800,800,800,22000,22000,22000,101,101};          //随机间隔点击时间最大值，单位ms
...
int initAllFirstDelayTime()
{
  ...
  for(int i = 0;i < 11;i++)
  {
      ...
      pinRD[i] = nowtime + random(delay_Mins[i],delay_Maxs[i]);
  }
}
int updateNowTouch(int pinID)
{
  unsigned long nowtime=millis();  //系统开始运行的时间;
  pinRD[pinID] =  nowtime + random(delay_Mins[pinID],delay_Maxs[pinID]);
}
